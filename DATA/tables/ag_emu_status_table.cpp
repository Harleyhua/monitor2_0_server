...
//硬件版本
const QString ag_emu_status_table::c_field_hard_version = "hard_version";
...
{
    QString select_cmd_head = QString("SELECT %1,%2,%3,%4,%5,%6,%7,%8 FROM ")
                            .arg(c_field_emu_cid,c_field_status,c_field_status_reserve,c_field_run_mode,
                                 c_field_sign,c_field_func,c_field_sys_time,c_field_hard_version) + m_name + " ";
    ...
    if(query.exec(select_cmd_head + condition))
    {
        ...
        while (query.next()) {
            ...
            //硬件版本
            emu_data_js.insert(c_field_hard_version,query.value(c_field_hard_version).toString());
